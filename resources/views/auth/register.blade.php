...
<head>
  <meta charset="utf-8">
  <title>JurassiDraft – {{ __('Create account') }}</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  @vite(['resources/css/app.css', 'resources/js/app.js'])
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css">
</head>

<body class="min-h-screen bg-gradient-to-br from-emerald-700 via-emerald-800 to-gray-900 flex items-center justify-center px-4 py-4 bg-emerald-900">
  ...
  <div class="w-full max-w-sm space-y-4">
    ...
    <div class="rounded-xl border border-white/10 bg-white/10 backdrop-blur-md shadow-lg p-5">
      <form method="POST" action="{{ route('register.post') }}" novalidate class="space-y-4">
        ...
        <h2 class="text-lg font-semibold text-white text-center">{{ __('Create account') }}</h2>
        ...
        <!-- Name -->
        <div>
          <label for="nombre" class="block text-xs font-medium text-emerald-100 mb-1">{{ __('Name') }}</label>
          <input
            id="nombre"
            name="nombre"
            type="text"
        ...
        <!-- Username -->
        <div>
          <label for="nickname" class="block text-xs font-medium text-emerald-100 mb-1">{{ __('Username') }}</label>
          <input
            id="nickname"
            name="nickname"
            type="text"
        ...
        <!-- Password -->
        <div>
          <label for="contrasena" class="block text-xs font-medium text-emerald-100 mb-1">{{ __('Password') }}</label>
          <div class="relative">
            <input
              id="contrasena"
              name="contrasena"
        ...
        <!-- Confirm password -->
        <div>
          <label for="contrasena_confirmation" class="block text-xs font-medium text-emerald-100 mb-1">{{ __('Confirm password') }}</label>
          <div class="relative">
            <input
