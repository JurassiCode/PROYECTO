...
<head>
  <meta charset="utf-8">
  <title>PLAY · Agregar jugadores</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  @vite(['resources/css/app.css','resources/js/app.js'])
</head>

<body class="min-h-[100dvh] bg-gray-50">
  <div class="mx-auto max-w-3xl px-4 py-8">
    <div class="mb-6 flex items-center gap-3">
      <img src="{{ asset('images/logojuego_nobg.png') }}" class="h-10 w-auto" alt="Logo">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="lg:col-span-2 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <p class="mt-2 text-xs text-gray-500">Se valida contra la tabla <code>usuarios</code> (por <code>id_usuario</code> o <code>usuario</code>). Máximo 6.</p>
      </div>
      ...
      <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div class="mt-3 grid grid-cols-1 gap-2">
          <form method="POST" action="{{ route('play.clear') }}">
            @csrf
            <button class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm hover:bg-gray-50">
              Vaciar lista
            </button>
          </form>
          <form method="POST" action="{{ route('partidas.store') }}" class="space-y-2">
            ...
            <button class="w-full inline-flex items-center justify-center rounded-md bg-emerald-600 px-3 py-2 text-white text-sm shadow-sm hover:bg-emerald-700">
              Crear partida
            </button>
          </form>
    ...
    <div class="mt-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm flex items-center justify-between">
          <div>
            <div class="text-sm font-medium">{{ $j['nombre'] ?: $j['usuario'] }}</div>
            <div class="text-xs text-gray-500">ID: {{ $j['id_usuario'] }} · Usuario: {{ $j['usuario'] }}</div>
          </div>
          <form method="POST" action="{{ route('play.remove', $j['id_usuario']) }}">
            @csrf
            <button class="rounded-md border border-red-300 px-2 py-1 text-xs text-red-700 hover:bg-red-50">
              Quitar
            </button>
          </form>
        </div>
