<article class="rounded-2xl border border-emerald-800/60 bg-emerald-950/40 shadow-sm ring-1 ring-emerald-500/10">
  <div class="px-5 py-4 border-b border-emerald-800/60 flex items-center justify-between">
    <h2 class="text-emerald-100 font-semibold text-sm tracking-wide">{{ __('Add placement') }}</h2>
  </div>
  <form method="POST" action="{{ route('partidas.agregarColocacion', $partida->id) }}" class="p-5 grid grid-cols-3 gap-3">
    @csrf
    <div>
      <label class="block text-[11px] text-emerald-300/80 mb-1">{{ __('Player') }}</label>
    ...
    <div>
      <label class="block text-[11px] text-emerald-300/80 mb-1">{{ __('Dinosaur') }}</label>
    ...
    <div>
      <label class="block text-[11px] text-emerald-300/80 mb-1">{{ __('Enclosure') }}</label>
