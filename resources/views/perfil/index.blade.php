...
<section
  ...
  <div class="max-w-5xl mx-auto px-6 space-y-10">
    ...
    <header class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        ...
        <h2 class="text-base text-emerald-100/90">
          {{ __('User:') }}
          <span class="font-semibold text-emerald-200">{{ $user->nickname }}</span>
          <span class="text-gray-400">#{{ $user->id }}</span>
        </h2>
        <p class="text-sm text-gray-400 mt-1">
          {{ __('Member since') }} {{ $user->creado_en->format('d/m/Y') }}
        </p>
      </div>
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
      <div class="bg-emerald-800/30 hover:bg-emerald-800/50 p-5 rounded-xl shadow-sm transition">
        ...
        <p class="text-sm text-gray-300 tracking-wide mt-1">{{ __('Games played') }}</p>
      </div>
      <div class="bg-emerald-800/30 hover:bg-emerald-800/50 p-5 rounded-xl shadow-sm transition">
        <p class="text-4xl font-extrabold text-emerald-200">{{ $stats['creadas'] }}</p>
        <p class="text-sm text-gray-300 tracking-wide mt-1">{{ __('Games created') }}</p>
      </div>
      <div class="bg-emerald-800/30 hover:bg-emerald-800/50 p-5 rounded-xl shadow-sm transition">
    ...
    <section>
      ...
      <a href="{{ route('resultados.partida.show', $pj->partida->id ?? 0) }}"
        ...
        <span class="text-xs text-gray-400">{{ __('Points: :points', ['points' => $pj->puntos_totales]) }}</span>
      </a>
      @empty
      <p class="text-sm text-gray-500 italic">{{ __('You haven’t played any games yet.') }}</p>
      @endforelse
    </section>
    ...
    <section>
      ...
      @empty
      <p class="text-sm text-gray-500 italic">{{ __('You haven’t created any games yet.') }}</p>
      @endforelse
    </section>
  </div>
  ...
  <div
    ...
    <div
      ...
      <h2 class="text-xl font-bold text-emerald-300 mb-4">{{ __('Edit Profile') }}</h2>
      ...
      <form method="POST" action="{{ route('perfil.update') }}" class="space-y-4">
        ...
        {{-- Nombre --}}
        <div>
          <label class="block text-sm text-gray-300 mb-1">{{ __('Name') }}</label>
        ...
        {{-- Contraseña --}}
        <div>
          <label class="block text-sm text-gray-300 mb-1">{{ __('New password (optional)') }}</label>
        ...
        {{-- Confirmar --}}
        <div>
          <label class="block text-sm text-gray-300 mb-1">{{ __('Confirm password') }}</label>
