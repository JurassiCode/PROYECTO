...
<body class="bg-gradient-to-br from-emerald-700 via-emerald-800 to-gray-900 text-gray-100 antialiased min-h-screen flex flex-col bg-emerald-900">
  ...
  <!-- Navbar -->
  <nav x-data="{ open:false, userMenu:false, langMenu:false }"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 items-center justify-between">
        ...
        <a href="{{ route('home') }}"
          ...
          <span class="tracking-tight text-lg">{{ __('JurassiDraft') }}</span>
        </a>
        ...
        <div class="hidden lg:flex items-center gap-4">
          ...
          $mainActionLabel = $isAdmin ? __('Admin Panel') : __('Play');
          ...
          <div x-data="{ open: false }" class="relative ml-2">
            <button @click="open = !open"
              class="flex items-center gap-2 text-2xl text-gray-300 hover:text-emerald-300 transition-all duration-200 select-none
              ...
              <i class="bi transition-transform duration-200"
                :class="open ? 'bi-chevron-up rotate-360 text-emerald-300' : 'bi-chevron-down text-gray-400 group-hover:text-emerald-300'"></i>
            </button>
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 text-gray-300 text-sm">
      <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-4">
        <div class="text-center md:text-left">
          © {{ date('Y') }} <strong class="text-white">{{ __('JurassiDraft') }}</strong> — {{ __('All rights reserved') }}.
        </div>
