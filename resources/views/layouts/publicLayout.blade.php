  ...
  <nav x-data="{ open:false, userMenu:false }"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="flex h-16 items-center justify-between">
        <!-- Brand -->
        <a href="{{ route('home') }}" class="group flex items-center font-bold text-gray-900">
          <img src="{{ asset('images/logojuego_nobg.png') }}" alt="JurassiDraft Logo"
            class="h-10 w-10 object-contain mr-2 rounded-xl">
          <span class="tracking-tight">JurassiDraft</span>
        </a>
        ...
        <div class="hidden lg:flex items-center gap-3">
          ...
          $mainBtnIcon = $isAdmin ? 'bi-speedometer2' : 'bi-play-fill';
          @endphp

          <!-- Botón principal con aura -->
          <a href="{{ $mainActionUrl }}"
            class="inline-flex items-center gap-2 rounded-md px-4 py-2 text-white shadow-sm
          ...
          <div class="relative" @click.outside="userMenu=false">
            <button @click="userMenu=!userMenu"
              class="flex items-center rounded-full focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 transition"
            ...
            <ul x-cloak x-show="userMenu" x-transition
              class="absolute right-0 mt-2 w-56 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-lg shadow-emerald-900/5">
              ...
              <li>
                <a href="{{ $mainActionUrl }}"
                  ...
                  // color del link (y del icono via text-current): 'text-amber-600'=> $isAdmin,
                  'text-emerald-600' => ! $isAdmin,
                  ])>
                  <i class="bi {{ $mainBtnIcon }} mr-2 text-current"></i>
                  {{ $mainActionLabel }}
                </a>
              </li>
          ...
          @else
          <!-- Registrarse: chip claro -->
          <a href="{{ route('register') }}"
            class="inline-flex items-center gap-2 rounded-md bg-white border border-emerald-300 text-emerald-700
                      px-4 py-2 text-sm font-medium shadow-sm
                      hover:bg-emerald-50 hover:border-emerald-400 hover:text-emerald-800
                      focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-300/40
                      active:scale-[0.98] transition-all duration-200">
            Registrarse
          </a>

          <!-- Iniciar sesión: botón aura emerald -->
          <a href="{{ route('login') }}"
            class="inline-flex items-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-white shadow-sm
                      hover:bg-emerald-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-600/40
                      active:scale-[0.98] transition-all duration-200">
            Iniciar sesión
          </a>
          @endauth
    ...
    <div id="navMain" x-cloak x-show="open" x-transition class="lg:hidden border-t border-gray-200 bg-white">
      <div class="mx-auto max-w-7xl px-4 py-3">
        ...
        <div class="flex flex-col gap-2">
          <a href="{{ route('register') }}"
            class="inline-flex items-center gap-2 rounded-md bg-white border border-emerald-300 text-emerald-700
                      px-4 py-2 text-sm font-medium shadow-sm hover:bg-emerald-50 hover:border-emerald-400 hover:text-emerald-800
                      focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-300/40 transition-all duration-200">
            Registrarse
          </a>
          <a href="{{ route('login') }}"
            class="w-full rounded-md bg-emerald-600 px-4 py-2 text-center text-white hover:bg-emerald-700
                      focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-600/40 transition-all duration-200">
            Iniciar sesión
          </a>
        </div>
        @endauth
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
      <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-4">
        ...
        <div class="text-center md:text-left text-gray-500">
          © {{ date('Y') }} <strong class="text-gray-900">JurassiDraft</strong> — Derechos reservados.
        </div>
