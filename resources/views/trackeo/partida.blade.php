...
@section('content')
    <!-- Page header (dentro del main del layout) -->
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                            @forelse($datos['restric']['tags'] as $tag)
                                <span
                                    class="rounded-md bg-gray-100 text-gray-700 text-xs px-2 py-1">{{ $tag }}</span>
                            @empty
                                <span class="rounded-md bg-gray-100 text-gray-400 text-xs px-2 py-1">Sin
                                    restricciones</span>
                            @endforelse
                        </div>
                    </div>
            ...
            <div class="mt-3">
                @if (empty($datos['jugadores']))
                    <div class="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                        No hay jugadores cargados. Agregalos desde <a href="{{ route('play') }}"
                            class="underline">/play</a>.
                    </div>
                @else
                            ...
                            @foreach ($datos['jugadores'] as $p)
                                ...
                                <article class="rounded-lg border border-gray-200 bg-white p-3 shadow-sm">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="h-8 w-8 rounded-full {{ $avatar }} flex items-center justify-center text-sm font-semibold">
                                                {{ strtoupper(substr($p['nombre'], 0, 1)) }}
                                            </div>
                                            <h3 class="text-sm font-semibold">{{ $p['nombre'] }}</h3>
                                        </div>
                                        <span
                                            class="rounded-full {{ $badge }} text-xs px-2 py-0.5">{{ $p['estado'] }}</span>
                                    </div>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                ...
                <div class="mt-3 grid grid-cols-2 gap-2">
                    <!-- Placeholders -->
                    <span
                        class="rounded-md bg-emerald-600 px-3 py-2 text-white text-sm shadow-sm text-center select-none opacity-60">
                        Siguiente turno
                    </span>
                    <span
                        class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm text-center select-none opacity-60">
                        Finalizar ronda
                    </span>
                    <span
                        class="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm text-center select-none opacity-60 col-span-2">
                        Reiniciar partida
                    </span>
                ...
                <p class="mt-3 text-xs text-gray-500">
                    Placeholder: algunos botones aún no tienen lógica implementada.
                </p>
            </div>
        </section>
    </div>
