...
@section('title', __("Game #:id (:name)", ['id' => $partida->id, 'name' => $partida->nombre]))
...
<section class="min-h-[100dvh] relative flex flex-col items-center justify-start py-16 px-6 text-gray-100">
  ...
  <div class="relative text-center mb-14">
    ...
    <p class="mt-3 text-emerald-100/90 text-lg">
      {{ __('Game #:id (:name) finished', ['id' => $partida->id, 'name' => $partida->nombre]) }}
    </p>
  </div>
    ...
    <table class="w-full text-center">
      <thead class="bg-emerald-900/70 text-emerald-200 uppercase text-sm">
        <tr>
          ...
          <th class="px-4 py-3">{{ __('Points') }}</th>
        </tr>
      </thead>
  ...
  <div class="relative z-10 mt-12 flex flex-wrap justify-center gap-4">
    <a href="{{ route('lobby') }}"
      class="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-lg font-semibold rounded-xl shadow-md transition hover:shadow-emerald-400/30">
      🔄 {{ __('Play again') }}
    </a>
    <a href="{{ route('home') }}"
